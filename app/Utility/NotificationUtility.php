...
class NotificationUtility
{
    ...
    public static function sendApprovalRFQ($order, $request = null)
    {       
        //sends email to customer with the invoice pdf attached
        $array['view'] = 'emails.rfq_request';
        $array['subject'] = translate('Your request for quote');
        $array['from'] = env('MAIL_FROM_ADDRESS');
        $array['link'] = route('rfq_request_user.index');
        $customer_id = $order->buyer_id;
        $user_data = User::find($customer_id);
        $order->customer_name = $user_data->name;
        $array['order'] = $order;        
        try {
            if($user_data->email != null)
            {
                // Mail::to($user_data->email)->queue(new SecondEmailVerifyMailManager($array));
            }
            // Mail::to($order->orderDetails->first()->product->user->email)->queue(new InvoiceEmailManager($array));
        } catch (\Exception $e) {
        }
